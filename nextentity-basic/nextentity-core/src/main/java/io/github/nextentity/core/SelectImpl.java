...
public class SelectImpl<T> extends WhereImpl<T, T> implements Select<T>, SelectFetchStep<T> {
    ...
    public SelectImpl(QueryConfig config, Class<T> type) {
        super(config, QueryStructure.of(type));
    }
    ...
    protected RowsSelectWhereStep<T, Tuple> select(boolean distinct, ExpressionNode... nodes) {
    ...
    protected <R extends Tuple> RowsSelectWhereStep<T, R> selectTuple(boolean distinct, Path<?, ?>... paths) {
