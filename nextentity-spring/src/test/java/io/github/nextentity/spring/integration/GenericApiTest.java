...
public class GenericApiTest {
    ...
    public void testIfNull(UserRepository userQuery) {
        ...
        Predicate<User> predicate = id.eqIfNotNull(null)
                .and(predicates);
        List<User> list1 = userQuery.where(predicate).list();
        log.info("{}", list1.size());
        list1 = userQuery.where(predicate.and(User::getId).eq(1)).list();
        log.info("{}", list1.size());
        list1 = userQuery.where(predicate.and(id.eq(1))).list();
        log.info("{}", list1.size());
    ...
    public void testOperatorIfNotNull(UserRepository userQuery) {
        ...
        qList = userQuery.where(Path.of(User::getRandomNumber).eqIfNotNull(20)).list();
        ...
        qList = userQuery.where(predicate).list();

        assertEquals(qList, fList);
        qList = userQuery.where(Path.of(User::getRandomNumber).gtIfNotNull(20)).list();
        fList = userQuery.users().stream().filter(u -> u.getRandomNumber() > 20)
                .collect(Collectors.toList());
        assertEquals(qList, fList);
        qList = userQuery.where(Path.of(User::getRandomNumber).geIfNotNull(20)).list();
        fList = userQuery.users().stream().filter(u -> u.getRandomNumber() >= 20)
                .collect(Collectors.toList());
        assertEquals(qList, fList);
    ...
    public void testResultBuilder(UserRepository userQuery) {
        ...
        first = userQuery.where(Path.of(User::getId).eq(0)).single();
        assertEquals(first, userQuery.users().get(0));
    ...
    public void testAttr(UserRepository userQuery) {
        User first = userQuery.orderBy(Path.of(User::getId).desc()).first();
        ...
        first = userQuery.orderBy(Path.of(User::isValid).desc(), Path.of(User::getId).asc()).first();
        users = new ArrayList<>(userQuery.users());
